...
from scrapy_learn1.items import StockinfoItem, CsdnblogcrawlspiderItem, CforumTopicInfo, CforumReplyInfo, Ips
...
class MySQLStoreCnblogsPipeline(object):
    ...
    def _process_cforumTopicinfo(self, conn, item, spider):
        ''' 判断url是否存在；new标签是True'''
        # linkmd5id = self._get_linkmd5id(item)
        # linkmd5id = item['url']
        now = strftime("%Y-%m-%d %H-%M-%S", localtime())
        # conn.execute(" SELECT * FROM cforum_topicinfo  WHERE url =%s", (linkmd5id,))
    ...
    def _process_Ips(self, conn, item, spider):
        try:
            conn.execute("insert into ips VALUES (%s,%s)", (item["ip"], item["port"]))
        except Exception, e:
            logger.error(e)
            pass
        pass
